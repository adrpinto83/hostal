...
def upgrade():
    ...
    room_status = sa.Enum('available', 'occupied', 'cleaning', 'maintenance', 'out_of_service', name='room_status')
    room_type = sa.Enum('single', 'double', 'suite', name='room_type')
...
def downgrade():
    ...
    op.drop_table("users")
    sa.Enum(name='room_status').drop(op.get_bind(), checkfirst=False)
    sa.Enum(name='room_type').drop(op.get_bind(), checkfirst=False)
    # ### end Alembic commands ###
