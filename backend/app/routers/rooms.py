...
def list_rooms(
    ...
    q: Optional[str] = Query(None, description="Buscar por número o notas (ILIKE)"),
    type: Optional[str] = Query(None, pattern="^(single|double|suite)$"),
    skip: int = 0,
    ...
    if type:
        query = query.filter(Room.type == RoomType(type))
